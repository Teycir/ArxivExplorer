// lib/hooks.ts — ported from SeekYou
...
export function useTextScramble(
  ...
  const scramble = useCallback(() => {
    ...
    scramblerRef.current.scramble(text, setDisplayText, () => {
  ...
  return { displayText, isScrambling, scramble, stop: () => scramblerRef.current?.stop() };
}
