...
function format_paper(paper: any, verbose = false) {
  const lines = [
    `ID: ${paper.id}`,
    `Title: ${paper.title}`,
  ...
  if (verbose && paper.summary) {
    if (paper.summary.paperType) lines.push(`Type: ${paper.summary.paperType}`);
    if (paper.summary.keywords) lines.push(`Keywords: ${paper.summary.keywords.join(', ')}`);
  }
...
async function cmd_search(query: string, opts: any = {}) {
...
async function cmd_paper(id: string) {
  ...
  if (data.summary) {
    ...
    console.log(`\nKey Contributions:\n${data.summary.keyContributions.map((k: string) => `- ${k}`).join('\n')}`);
    console.log(`\nMethods:\n${data.summary.methods.map((m: string) => `- ${m}`).join('\n')}`);
    if (data.summary.prerequisites?.length) {
      console.log(`\nPrerequisites:\n${data.summary.prerequisites.map((p: string) => `- ${p}`).join('\n')}`);
    }
  }
}
...
async function cmd_topic(slug: string, limit = 10) {
