...
export function AbstractSearch({ onSearch }: { onSearch?: () => void }) {
  ...
  return (
    <div className={onSearch ? '' : 'mb-6'}>
      {/* Toggle button only shown when used standalone (not inside navbar popover) */}
      {!onSearch && (
        <button
      ...
      {(isOpen || onSearch) && (
        <div className={onSearch ? '' : 'border border-neon-red/10 rounded-lg p-4 bg-black/20'}>
